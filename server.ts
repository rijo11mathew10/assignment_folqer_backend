...
// Example route to fetch all users (placeholder)
app.get('/reports', (c) => {
  try {
    ...
    // Now, create a final array of reports for each work_year, calculating the average salary
...
// Example route to fetch a specific user by ID
app.get('/reports/:year', (c) => {
    ...
    const filteredData = data.filter((row) => {
        const rowYear = row.work_year; // Convert row.work_year to string
        console.log(`Comparing ${rowYear} with ${year}`);  // Log the comparison
        return rowYear === year;
      });
